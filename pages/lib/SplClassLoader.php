...
class SplClassLoader {
	...
	public function loadClass( $className ) {
		$this->printdebug($className);
		$isFound = false;
		$this->printdebug("loadClass className=".$className." _namespace=".$this->_namespace);
		$this->printdebug("loadClass className=".$className." _namespaceSeparator=".$this->_namespaceSeparator);
		...
		if ( null === $this->_namespace || $this->_namespace.$this->_namespaceSeparator === substr( $className, 0, strlen( $this->_namespace.$this->_namespaceSeparator ) ) ) {
			...
			if ( false !== ( $lastNsPos = strripos( $className, $this->_namespaceSeparator ) ) ) {
				...
				$this->printdebug("fileName=".$fileName);
			}
			$fileName .= str_replace( '_', DIRECTORY_SEPARATOR, $className ) . $this->_fileExtension;
			$this->printdebug("fileName 2=".$fileName);
			$includePaths = $this->_includePaths ?: array( '.' );
			foreach ( $includePaths as $includePath ) {
				$unresolvedFilePath = $includePath . DIRECTORY_SEPARATOR . $fileName;
				$this->printdebug("loadClass className=".$className." includePath=".$includePath);
				$this->printdebug("loadClass className=".$className." DIRECTORY_SEPARATOR=".DIRECTORY_SEPARATOR);
				$this->printdebug("loadClass className=".$className." fileName=".$fileName);
		...
		$this->printdebug("loadClass className=".$className." isFound=".$isFound);
	...
	private function tryLoadClassByPath( $className, $unresolvedFilePath ) {
		$this->printdebug("tryLoadClassByPath className=".$className." unresolvedFilePath=".$unresolvedFilePath);
